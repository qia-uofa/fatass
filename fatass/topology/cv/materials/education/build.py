...
def build(current_cv: CurrentCv):
    fatass.free(
        ...
        prompt=(
            ...
            f"If the CV is present but has no education section (degrees, "
            f"schools, etc.), also do nothing and write nothing — do not "
            f"create `from_current_cv.md`.\n\n"
            f"Otherwise, extract every degree mentioned in the education "
            f"section: degree earned, institution, advisor, dissertation/"
            f"thesis title, and date, for each one. Extract only what the "
            f"CV actually states — never infer or add a detail it doesn't "
            f"mention; omit any of these fields for a given degree if the "
            f"CV doesn't state it.\n\n"
            f"Write the extracted entries to `from_current_cv.md` in the "
            f"current directory, one entry per degree. The candidate may "
