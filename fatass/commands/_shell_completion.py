...
def _complete_asset_path(node_expr: str, rel: str) -> list[str]:
    ...
    target, as full replacements for `rel` itself. `rel/path`'s own
    internal separator stays "/" — only the outer node/rel boundary
    changed from "/" to "("/")"."""
    try:
...
class ShellCompleter(Completer):
    ...
    def get_completions(self, document: Document, complete_event: CompleteEvent):
        ...
        if "(" in word:
            node_expr, _, rest = word.partition("(")
            rel = rest[:-1] if rest.endswith(")") else rest
