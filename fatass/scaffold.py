...
from ._paths import NODES_ROOT as _NODES_ROOT
from ._paths import TOPOLOGY_ROOT as _TOPOLOGY_ROOT
from .errors import TopologyValidationError
from .free import free_topology
...
_TRANSFORMS_INIT_PY = """from fatass._import_tree import import_transforms
...
def refine_transform(node_path: str, transform_name: str, prompt: str) -> None:
    ...
    full_prompt = (
        ...
        f"{prompt}. It must define a function named {transform_name} with "
        f"at least one parameter type-annotated with a Node subclass "
        f"imported from fatass.topology.<dependency node path> — see other "
        f"nodes under the provided topology directory for examples of the "
        f"convention."
    )
