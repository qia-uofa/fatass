...
def wide_multi_sentiment_table(out: dict[str, list], items: list | None = None) -> pd.DataFrame:
    """Reshape `pipeline.run_multi_sentiment`'s output into one row per item.

    `out[name][i]` is guaranteed aligned with `out[other_name][i]` by
    construction — both are deep copies of the same shared Phase-0 trial
    (`plan_benzon.md`'s wrinkles #6/#7) — so this is a thin formatting
    convenience, not a defensive join. Pass the same `items` used to build
    `out` to tag each row with its `qid`/`question`/`meta` (e.g. Part 3's
    `meta["relation"]`, for a `groupby` caveated-vs-clean comparison).
    """
    names = list(out.keys())
    ...
    for i in range(n):
        ...
        if items is not None:
            row["qid"] = items[i].qid
            row["question"] = items[i].question
            row.update({f"meta_{key}": value for key, value in items[i].meta.items()})
        for name in names:
            trial = out[name][i]
            row["qid"] = row.get("qid", trial.qid)
            row[f"{name}_answer"] = trial.answer
            row[f"{name}_class_index"] = trial.class_index
            row[f"{name}_confidence"] = trial.confidence
            row[f"{name}_valid"] = trial.valid
        rows.append(row)
