...
def run_steering(
    ...
    if cfg.prompt_kind in ("categorical", "magistral"):
        # numeric_midpoints(cfg.prompt_kind) returns None here, which would make
        # intervention_metrics fall back to metrics.MIDPOINTS — CONFIDENCE's own
        # 10-class array. That's only correct by coincidence for cfg.sentiment is
        # CONFIDENCE; any other categorical sentiment (fewer/different classes)
        # needs its own class_midpoint, in its own class order.
        midpoints = np.array(
