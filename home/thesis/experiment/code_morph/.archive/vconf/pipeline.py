...
def run_phase0(
    ...
    for start in iterator:
        ...
        rendered = [
            render_prompt(
                tokenizer,
                P.build_phase0_prompt(
                    item.question, kind, sentiment=cfg.sentiment,
        ...
        for i, item in enumerate(chunk):
            ...
            if kind == "categorical":
                cls = P.parse_class(text[len(answer):], classes=cfg.sentiment.classes)
                trial.phase0_class = cls
                trial.phase0_confidence = cfg.sentiment.class_midpoint[cls] if cls else None
            else:
...
# --------------------------------------------------------------------------- #
# Sharing one Phase-0 pass across multiple sentiments (plan_benzon.md) —
# ``run_phase0`` bakes ``cfg.sentiment`` into the Phase-0 prompt itself, so
# running it once per sentiment on the same dataset could generate a
# *different* Phase-0 answer per sentiment for the same question.
# --------------------------------------------------------------------------- #


def run_multi_sentiment(
    loaded: LoadedModel,
    items: list[QuestionItem],
    base_cfg: RunConfig,
    sentiments: dict[str, SentimentSpec],
) -> dict[str, list[Trial]]:
    """One shared Phase-0 pass, one Phase-1 pass per sentiment over deep copies of it.

    Phase 0 runs exactly once, under the fixed reference sentiment
    ``CONFIDENCE`` (the pipeline's default everywhere else), then each
    sentiment's Phase-1 pass mutates its own deep copy of that trial set.
    This guarantees ``out[name1][i]`` and ``out[name2][i]`` share the same
    ``question``/``answer``/``answer_logprobs`` by construction, with no
    cross-run trial joining needed.
    """
    phase0_cfg = replace(base_cfg, sentiment=CONFIDENCE)
    base_trials = run_phase0(loaded, items, phase0_cfg)
    ...
    for name, sentiment in sentiments.items():
        ...
        run_phase1(loaded, trial_copies, cfg)
        out[name] = trial_copies
    return out
...
def render_phase1(loaded: LoadedModel, trial: Trial, cfg: RunConfig | None = None) -> RenderedPrompt:
    ...
    cfg = cfg or loaded.config
    built = P.build_confidence_prompt(
        trial.question, trial.answer, cfg.prompt_kind, sentiment=cfg.sentiment,
        strip_question_punctuation=cfg.strip_question_punctuation,
    )
...
def run_phase1(
    ...
    cfg = cfg or loaded.config
    batch_size = batch_size or cfg.batch_size
    ids = target_token_ids(loaded.tokenizer, cfg.prompt_kind, classes=cfg.sentiment.classes)
    ...
    for start in iterator:
        ...
        for i, trial in enumerate(chunk):
            ...
            if scale is None:
                trial.confidence = cfg.sentiment.class_midpoint[cfg.sentiment.classes[trial.class_index]]
            else:
