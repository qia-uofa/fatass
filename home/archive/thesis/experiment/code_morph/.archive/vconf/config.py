...
#: Phase-0 prepared datasets (`dataset_prep.py`) — procedurally generated seeds,
#: distinct from `DATA_RAW` (downloaded, unmodified source data).
...
# huggingface_hub resolves its cache directory from HF_HOME as a module-level
# constant at *import* time (huggingface_hub.constants.HF_HOME/HF_HUB_CACHE).
# load_credentials() used to only run inside models.load_model(), by which
# point transformers/huggingface_hub had already been imported (at the top of
# models.py) and had already locked in the OS default `~/.cache/huggingface`
# — so HF_HOME was always set too late to matter, and every checkpoint
# download silently landed in the home directory instead of $PROJECT_ROOT's
# .hf_cache, regardless of this module's own "nothing here downloads or
# creates anything" docstring above. vconf.config is always the first vconf
# submodule imported (vconf/__init__.py imports it eagerly; vconf.models is
# not), so calling this here — a plain env-var default, no filesystem writes
# — is early enough to actually take effect before anything imports
# transformers.
load_credentials()
...
class RunConfig:
    ...
    dataset: str = "triviaqa"  # triviaqa | bigmath | mmlu | benzon:<name>
    #: What verbally-reported property is elicited (vconf.sentiment.SentimentSpec)
    #: and what it's checked against (vconf.ground_truth.GroundTruth) — default
    #: to the manual's own instance, confidence vs. correctness.
    ...
    #: Opt-in for the exploratory PQNL position (`prompts._qa_block`) — drops the
    #: question's own trailing punctuation in the Phase-1 prompt only, so PQNL's
    #: newline isolates instead of merging into "?\n"/".\n". False everywhere
    #: except a run explicitly investigating PQNL: every other position (PANL,
    #: CC, ...) and the manual's own verbatim CONFIDENCE prompt are unaffected
    #: either way, since `_qa_block` only touches the question text.
    ...
    def __post_init__(self) -> None:
        # Deferred import: vconf.ground_truth imports vconf.grading, which
        # imports this module — importing it at module load time here would
        # be circular. By the time a RunConfig is actually instantiated,
        # config.py has finished loading, so this is safe.
        if self.ground_truth is None:
            from .ground_truth import AliasCorrectness

            self.ground_truth = AliasCorrectness()
