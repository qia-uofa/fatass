...
#: The positions defined in §2.6, plus PQNL (post-question newline) -- an
#: exploratory position outside the paper's own set, added to test whether a
#: sentiment (e.g. nuance) is already represented right after the question
#: itself, before the model has even produced an answer.
POSITION_NAMES = (
    "AC", "first A", "last A", "PANL", "PANL+1", "FCC", "CC", "QTT", "PQNL",
)
...
def locate_positions(
    ...
    if "PQNL" in spans:
        positions["PQNL"] = char_to_token(offsets, spans["PQNL"][0])
    return positions
