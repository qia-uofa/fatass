...
def run_experiment0(
    ...
    grader = grader or (grading.gpt4o_mini_grader if grading.openai_available() else grading.alias_match_grader)
    ...
    histogram = M.class_histogram(np.array([t.class_index for t in trials], dtype=int))
    if cfg.prompt_kind in ("numeric", "minimal_numeric"):
...
def class_distribution(trials: list[Trial]) -> dict[str, float]:
    """Share of trials in each confidence class (§3.3 "Distribution")."""
    counts = M.class_histogram(np.array([t.class_index for t in trials], dtype=int))
